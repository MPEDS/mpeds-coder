...
def generateSample(n, db_name = None, pass_number = '1'):
...
def assignmentToCoders(article_ids, coder_ids, pass_number = '1'):
	...
	for coder_id in coder_ids:
		for a in article_ids:
			if pass_number == '1':
				to_add.append( ArticleQueue(article_id = a, coder_id = coder_id) )
			elif pass_number == '2':
				to_add.append( SecondPassQueue(article_id = a, coder_id = coder_id) )
			elif pass_number == 'ec':
				to_add.append( EventCreatorQueue(article_id = a, coder_id = coder_id) )
